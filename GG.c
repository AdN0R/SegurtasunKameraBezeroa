...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout, "\nOngi etorri segurtasun kameraren gestioaren aplikaziora.\n\nSar ezazu erabiltzaile izena:   ");

	// Erabiltzailea jaso.

	while(fgets(buf, MAX_BUF, stdin) == NULL)
	{
		fprintf(stdout,"Erabiltzaile izena beharrezkoa da.\n");
	}
...
//NIREA HASI
	...
	sprintf(buf,"USER$%s",buf);

	fprintf(stdout,"Hau da gorde dena: %s .",buf);

	if(write(sock, buf, strlen(buf)) < strlen(buf))
	{
		perror("Errorea erabiltzaile izena zerbitzarira bidaltzean.");
		exit(1);
	}

	if((n = read(sock, buf, MAX_BUF)) < 0)
	{
		perror("Errorea erabiltzaile izenaren erantzuna jasotzean.");
		exit(1);
	}
	...
	while(fgets(buf, MAX_BUF, stdin) == NULL)
	{
		fprintf(stdout,"Pasahitza beharrezkoa da.\n");
	}
